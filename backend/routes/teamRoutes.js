...
const { compressAndUploadImage, deleteImageFromCloudinary, extractPublicIdFromUrl } = require('../utils/imageUpload');
...
router.post('/register', authMiddleware, async (req, res) => {
  try {
    ...
    if (teamSize === 'Solo' && memberCount > 0) {
      return res.status(400).json({ message: 'Solo teams cannot have additional members' });
    }
    if (teamSize === 'Duo' && memberCount !== 1) {
      return res.status(400).json({ message: 'Duo teams must have exactly 1 additional member' });
    }
    if (teamSize === 'Team' && (memberCount < 2 || memberCount > 4)) {
      return res.status(400).json({ message: 'Teams must have 2-4 additional members' });
    }
    ...
    if (members && members.length > 0) {
      ...
      if (memberUsers.length !== members.length) {
        return res.status(400).json({ message: 'One or more members not found. Please ensure all members are registered on the platform.' });
      }
...
router.post('/upload-payment', authMiddleware, upload.single('paymentScreenshot'), async (req, res) => {
  try {
    ...
    if (!uploadResult.success) {
      return res.status(500).json({ message: 'Failed to process and upload image' });
    }
